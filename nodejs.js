...
client.on("ready", () => {
	...
	let status = " Aktif!";
	if (db.get("owosystemstatus") == "0") { //made by ToxiL, software version 8.5
		status = "Aktif Değil";
	}
	console.log("//made by ToxiL, software version 8.5 OwoHunt Bot Çalışıyor | Anlık durum: "+status);
...
setInterval(function() {
	if (Number(db.get("securitytime")) > 10 || Number(db.get("securitytime")) < 1) {
		...
		client.channels.get(db.get("channelid")).send("Güvenlik ayarları hatalı girildiğinden tekrar yapılandırıldı."); //made by ToxiL, software version 8.5
	}
	if (db.get("captchasecurity") == "1") {
		if (db.get("owosecurity") == "0") {
			...
			if (db.get("owosystemstatus") == "0") {
				...
				client.channels.get(db.get("channelid")).send("Bot tekrardan aktif edilmiştir."); //made by ToxiL, software version 8.5
				return;
			}
		}
	}
...
setInterval(function() {
	if (db.get("owosystemstatus") == "1") {
		if (db.get("autosell") == "1") {
			client.channels.get(db.get("channelid")).send("Otomatik satım işlemi yapılıyor.").then(msg => { //made by ToxiL, software version 8.5
				setTimeout(function() {
...
client.on("message", message => {
	...
	let huntdurum = "Aktif!";
	if (db.get("systems.whunt") == "0") { //made by ToxiL, software version 8.5
		...
		if (message.content.indexOf(client.user.username) != "-1") {
			...
			if (db.get("owomessagetype") == "çek") {
				if (Number(cash) < 3000) {
					...
					db.set("owomessagetype", "0"); //made by ToxiL, software version 8.5
...
client.on("message", message => {
	...
	if (message.channel.type == "dm") {
		...
		if (message.content.indexOf("Beep Boop. Are you a real human? Please reply with the following") != "-1") {
			...
			db.set("owosystemstatus", "0");
			db.set("owosecurity", "1"); //made by ToxiL, software version 8.5
			return;
		}
...
client.on("message", message => {
	...
	if (message.content.split(" ")[0] == "!lb") {
		...
		message.channel.send("owo lb all").then(msg => {
			msg.delete(); //made by ToxiL, software version 8.5
		});
	...
	if (message.content.split(" ")[0] == "!komut-ayarla") {
		if (message.content.split(" ")[1] == "whunt") {
			...
			return; //made by ToxiL, software version 8.5
		}
...
//made by ToxiL, software version 8.5